...
public final class CustomFireflySupport {
    private static Set<ResourceLocation> biomes = Set.of();
    private static Set<Block> blocks = Set.of();
    private static Map<ResourceLocation, Integer> biomeColors = Map.of();
    ...
    public static boolean isFireflyBiome(Holder<Biome> holder) {
        ...
        return holder.unwrapKey().map(ResourceKey::location).map(biomes::contains).orElse(false);
    }
    ...
    public static int resolveColor(Holder<Biome> holder, RandomSource random, int fallback) {
        ...
        if (!biomeColors.isEmpty()) {
            Integer c = holder.unwrapKey().map(ResourceKey::location).map(biomeColors::get).orElse(null);
            if (c != null) return c;
        }
