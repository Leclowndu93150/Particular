...
public class LeafParticle extends SingleQuadParticle {
    ...
    public void extract(QuadParticleRenderState renderState, Camera camera, float partialTick) {
        float ageDelta = Mth.lerpInt(partialTick, age, age + 1);
        if (age <= fadeInDuration) {
        ...
        if (expiring && ParticularConfig.COMMON.fallingLeavesLayFlatOnGround.get()) {
            ...
            rotationTop.rotateZ(Mth.lerp(partialTick, this.oRoll, this.roll));

            Quaternionf rotationBottom = new Quaternionf();
            rotationBottom.rotateX(-(float)Math.PI / 2.0f);
            rotationBottom.rotateZ(Mth.lerp(partialTick, this.oRoll, this.roll));
